...
（終わった記録です。挙げた直しどころは2026年9月22日までに全部直しました。今の状態は docs/plan.md を見てください）

（この資料の1から3に挙げた直しどころは、すべて実装済み。1と2は2026年9月21日、3は9月22日。IPの上限は管理画面の「同じ回線から1時間に◯回まで」で既定0（見ない）にし、Xの投稿文は告知ツイートのURLを外して画像カードを取り、禁止語は判定を返して表紙と目次に出さないだけにした）
...
（16件とも2026年9月22日に実装済み。傾きの視差はスマホでもパソコンでも止めず、傾きセンサーだけをやめてマウスの位置で動かす形にした。飾りのウィンドウは上30%より下に置かず、ウィンドウの層を薄くした）
