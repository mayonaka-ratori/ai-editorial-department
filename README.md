...
- 終わった記録（ペルソナでの見直し、全体確認、作る前のモック、使わなかった長い遊び方ページの案）: docs/archive/
...
- http://localhost:3000/guide が遊び方のページ。ブースのQRコードはここに向ける。
...
1. /admin で「受付中」にする。上限回数を確かめる。
   「同じ回線から1時間に」は0のままにする。会場のWi-Fiは大勢が同じ回線なので、数えると途中から送れなくなる。
2. 告知ツイートを投稿し、そのURLを /admin に入れる（入口の「告知を見る」になります）。
3. ブースのMacBookで /screen を全画面で開く。
4. 自分のスマホでブースのQRコードを読み、遊び方のページから結果まで1回通す。
5. 結果のURLをXの投稿画面に貼って、画像つきのカードが出るか見る。
   出なければ /r/{結果の番号}/opengraph-image を直接開いて、画像そのものが出るか確かめる。
